...
#
# NOTE: this file is not normally used directly. Students' real Makefile is
# the one cloned into ~/lab/ from chipcraft-lab-files (see setup.sh). This
# copy is kept in sync with that one only so it isn't stale dead code if
# anyone runs `make` from this repo's root by mistake.
#
